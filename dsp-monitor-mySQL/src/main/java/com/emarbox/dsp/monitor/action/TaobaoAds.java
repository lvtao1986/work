 ...
 * 淘宝静态创意广告代码生成服务
 */
@Controller("taobaoAdsAction")
public class TaobaoAds {
	
	@Autowired
	private TaobaoMonitorAction taobaoMonitorAction;

	private static final Logger log = LoggerFactory.getLogger(TaobaoAds.class);
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		try {
			...
			String clickUrl = "http://m.emarbox.com/taobao/click?info="
					...
					+ URLEncoder.encode(guid, "UTF-8") + "&ct="+ct + "&rt="+ rt+"&rt2="+rt2+"&bt="+bt+"&ut="+ut;
			...
			if (ctype.equals("image")) {
				this.imageCreative(writer, cwidth, cheight, csrc, clickUrl,
						 cmUrl,tanxclick);
			} else if (ctype.equals("flash")) {
				this.flashCreative(writer, cwidth, cheight, csrc, clickUrl,
						cmUrl,tanxclick);
			}else{
			...
			taobaoMonitorAction.impressionMonitor(info, price, guid, sign, apid, "", ct, rt, rt2,bt,ut,gid, req, resp); //记录展现
		} catch (Exception e) {
