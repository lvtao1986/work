...
@Controller("tencentMonitorAction")
@RequestMapping(value = "/tencent")
public class TencentMonitorAction extends BaseMonitorAction {

	private static final Logger log = LoggerFactory.getLogger(TencentMonitorAction.class);

	private static final Map<String, String> keyMap = new HashMap<String, String>(); // 存放解密秘钥
	...
	public void impressionMonitor(String info, String c, String kv, String gid,HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			String biddingPrice = decodePrice(c, kv);

			// boolean validate = monitorService.validInfo(infoStr, sign);
			// if (validate) {
...
//			writeLog(info,infoStr, sign, biddingPrice, guid, apid, campaignId,
//					creativeId, adUserId, projectId, biddingId, time,
//					adChannelId, costType, biddingRtbPrice, valuePrice, ctr,
//					division, biddingCost, commRate, taxRate, mediaSite,
//					userDomains, "impression", userId, ip, agent, referer);
			...
			impressionLog.setSupplier(SupplierInfo.TENCENT_ADX);
	...
	public String clickMonitor(String url, String info,String gid, HttpServletRequest request, HttpServletResponse response) {
	...
	public String decodePrice(String priceStr, String kv) {
		String key = keyMap.get(kv);
		if (key == null) {
			key = Config.getString("tencent.key." + kv);
			keyMap.put(kv, key);
		}
		...
		try {
			...
			String p = Tea.decryptString(priceStr, key);
			...
			log.warn("tencent价格解密失败,价格密文：" + priceStr + " error :" + e.getMessage(), e);
			return priceStr;
		}
	}
